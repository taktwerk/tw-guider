...
export abstract class DbBaseModel {
  ...
  private getCreateSQLQuery(): string {
    ...
    let rows = [this.secure(this.COL_ID) + ' INTEGER PRIMARY KEY AUTOINCREMENT'];
  ...
  public checkTableExit(TABLE_NAME) {
    return new Promise((resolve) => {
      if (this.dbIsBusy) {
        ...
        this.platform.ready().then(() => {
          this.db
            .query('SELECT COUNT(*) FROM ' + this.secure(TABLE_NAME))
            .then((res) => {
              this.dbIsReady = true;
              this.dbIsBusy = false;
              resolve(true);
  ...
  public isExistTable() {
    return new Promise((resolve) => {
      if (this.dbIsBusy) {
        ...
        this.platform.ready().then(() => {
          this.db
            .query('SELECT COUNT(*) FROM ' + this.secure(this.TABLE_NAME))
            .then((res) => {
              this.dbIsReady = true;
              this.dbIsBusy = false;
              resolve(true);
  ...
  public removeById(id: number): Promise<any> {
    return new Promise((resolve) => {
      this.dbReady().then((db) => {
        ...
        db.query('DELETE FROM ' + this.secure(this.TABLE_NAME) + ' WHERE ' + this.secure(this.COL_ID) + ' = ' + id)
          .then(() => {
            resolve(true);
          })
          .catch(() => {
    ...
    return new Promise((resolve) => {
      this.dbReady().then((db) => {
        if (db == null) {
          ...
          let query =
            'INSERT INTO ' +
            this.secure(this.TABLE_NAME) +
            ' (`' +
            this.columnNames().join('`, `') +
            '`) ' +
            'VALUES (' +
            this.columnValues().join(', ') +
            ') ';
    ...
    return new Promise((resolve) => {
      this.dbReady().then((db) => {
        if (db == null) {
          ...
          let query = 'DELETE FROM ' + this.secure(this.TABLE_NAME) + ' ' + 'WHERE ' + this.parseWhere(this.updateCondition);
          db.query(query)
            .then((res) => {
